...
//     </span>";
$timeAndSeen = "<span data-time='$created_at' class='message-time'>
        ".($isSender ? "<span class='fas fa-$seenIcon' seen'></span>" : '' )." <span >$msg_time</span></span>";
?>
<div class="message-card-wrapper @if($isSender) mc-sender @endif">
    ...
    <div class="message-card @if($isSender) mc-sender @endif" data-id="{{ $id }}">
        ...
        @if ($isSender)
            ...
            <div class="actions">
                <i class="fas fa-edit edit_btn" data-id="{{ $id }}" ></i>
            </div> 
            @endif
        @endif
        {{-- Card --}}
        <div class="message-card-content">
            @if (@$attachment->type != 'image' || $message)
                <div class="message">
                    <span class="type">{!! ($message == null && $attachment != null && @$attachment->type != 'file') ? $attachment->title : nl2br($message) !!}</span>
                    {!! $timeAndSeen !!}
