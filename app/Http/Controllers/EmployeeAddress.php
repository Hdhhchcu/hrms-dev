...
use App\Models\EmployeeAddress As ModelEmployeeAddress;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;

class EmployeeAddress extends Controller
{
    ...
    public function add(Request $request): RedirectResponse {
        ...
        $path="";
        $document = new ModelEmployeeAddress;
        if($request->hasFile('file')){
            $path = $request->file->store('images/employeeaddress/', 'public');
        }
    
        $document->emp_id=$request->emp_id;
        $document->address_type=$request->address_type;
        $document->file=$path;
        $document->country=$request->country;
        $document->state=$request->state;
        $document->city=$request->city;
        $document->pincode=$request->pincode;
    ...
    public function update(Request $request,ModelEmployeeAddress $id): RedirectResponse {
        $document=$id;
        if($request->hasFile('file')){
    ...
    //////edit employee
    public function edit(ModelEmployeeAddress $id){
        return $id;
    }


    //////delete employee
    public function delete(ModelEmployeeAddress $id):RedirectResponse{
        $id->delete();
        return redirect()->back();
    }
